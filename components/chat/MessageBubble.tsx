  ...
  const shareItems = [
    {
      ...
      icon: copied ? (
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#198754" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
      ) : (
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col py-1">
        {shareItems.map((item) => {
          const cls = "flex items-center gap-3 px-4 py-2 text-[0.8rem] text-[#475569] transition-colors hover:bg-[#f0faf4] hover:text-[#198754]";
          if (item.href) {
            return (
...
function ActionBar({ content }: { content: string }) {
  ...
  return (
    <div className="relative mt-1 flex gap-0.5">
      <button
        onClick={handleCopy}
        className="flex items-center gap-1 rounded-md px-2 py-1 text-[11px] text-[#94a3b8] transition-colors hover:bg-[#f0faf4] hover:text-[#198754]"
      >
        {copied ? (
  ...
  if (role === "user") {
    return (
      <div className="flex justify-end">
        <div className="max-w-[80%] rounded-2xl rounded-br-sm bg-[#198754] px-4 py-3 text-[0.9rem] leading-relaxed text-white shadow-sm sm:max-w-[70%]">
          {content}
        </div>
      </div>
    );
    ...
    <div className="flex gap-3">
      ...
      <div className="min-w-0 max-w-[90%] sm:max-w-[85%]">
        <div className="rounded-2xl rounded-tl-sm border border-[#e8ecf0] bg-white px-4 py-3 text-[0.9rem] leading-relaxed text-[#1e293b] shadow-[0_1px_3px_rgba(0,0,0,0.03)]">
          <div className="prose prose-sm prose-green max-w-none [&>*:first-child]:mt-0 [&>*:last-child]:mb-0 [&_p]:leading-relaxed [&_li]:leading-relaxed [&_h1]:text-[#0f1f14] [&_h2]:text-[#0f1f14] [&_h3]:text-[#0f1f14] [&_strong]:text-[#0f1f14] [&_a]:text-[#198754] [&_code]:rounded [&_code]:bg-[#f5f7fa] [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:text-[0.8rem]">
