...
function ShareMenu({ content, onClose }: { content: string; onClose: () => void }) {
  ...
  const shareItems = [
    {
      ...
      icon: copied ? (
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#198754" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
      ) : (
  ...
  return (
    <div
      ref={ref}
      className="absolute bottom-full left-0 z-50 mb-2 overflow-hidden rounded-xl border border-[#e2e8f0] bg-white shadow-[0_4px_24px_rgba(0,0,0,0.10)]"
    >
      <p className="border-b border-[#f0f0f0] bg-[#fafafa] px-4 py-2 text-[11px] font-semibold tracking-wide text-[#64748b] uppercase">Share</p>
      <div className="flex flex-col">
        {shareItems.map((item) => {
          const cls = "flex items-center gap-3 px-4 py-2.5 text-[13px] text-[#334155] transition-colors hover:bg-[#f0faf4] hover:text-[#198754]";
          if (item.href) {
            return (
              <a key={item.label} href={item.href} target="_blank" rel="noopener noreferrer" className={cls}>
                <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#f1f5f9]">{item.icon}</span>
                {item.label}
              </a>
            );
          }
