# AskZambia — Claude Code Instructions

> This file is the source of truth for building AskZambia.com.
...
**AskZambia** is Africa's first national AI knowledge platform. Users ask any question about Zambia — laws, government services, tourism, economy, culture, history, health, education — and receive accurate, cited answers powered by RAG (Retrieval-Augmented Generation) over a curated database of verified Zambian sources.
...
DATABASE_URL=postgresql://askzambia:STRONG_PASSWORD_HERE@localhost:5432/askzambia
DB_HOST=localhost
DB_PORT=5432
DB_NAME=askzambia
DB_USER=askzambia
...
BETTER_AUTH_URL=https://askzambia.com

# ─── APP ──────────────────────────────────────────────────────────────────────
NEXT_PUBLIC_APP_URL=https://askzambia.com
NEXT_PUBLIC_APP_NAME=AskZambia
...
NEXT_PUBLIC_POSTHOG_HOST=https://analytics.askzambia.com   # your own PostHog instance
...
This is the core of AskZambia. Follow this exact flow every time a user sends a message:
...
const openrouter = createOpenRouter({
  ...
  defaultHeaders: {
    ...
    'X-Title': 'AskZambia — National AI Knowledge Platform',
  },
})
...
export async function POST(req: Request) {
  ...
  const result = streamText({
    model: llm,
    system: ASKZAMBIA_SYSTEM_PROMPT.replace('{context}', context),
    messages,
    maxTokens: 1024,
...
services:
  ...
  app:
    ...
    environment:
      DATABASE_URL: postgresql://askzambia:${DB_PASSWORD}@postgres:5432/askzambia
  ...
  postgres:
    ...
    restart: unless-stopped
    environment:
      POSTGRES_DB: askzambia
      POSTGRES_USER: askzambia
      POSTGRES_PASSWORD: ${DB_PASSWORD}
    volumes:
      - postgres_data:/var/lib/postgresql/data
      - ./docker/postgres/init.sql:/docker-entrypoint-initdb.d/init.sql
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U askzambia"]
      interval: 10s
      timeout: 5s
      retries: 5
    networks: [askzambia-net]
...
The visual identity of AskZambia is based on Zambia's national colours.
...
- Sidebar: AskZambia logo, "New Chat" button, conversation history grouped by Today/Yesterday.
...
- Never show which LLM model is being used in the UI. Users see only the AskZambia brand.
...
1. `npx create-next-app@latest askzambia --typescript --tailwind --app --src-dir=false`
...
18. Build `app/page.tsx` landing page using `askzambia-landing.html` prototype as reference. CTA and suggestion chips navigate to `/chat?q=ENCODED_QUERY`.
...
23. Add domain `askzambia.com` — Coolify handles SSL automatically
24. Trigger first deploy — verify live at `https://askzambia.com`
...
*AskZambia — Africa's First National AI Knowledge Platform*
